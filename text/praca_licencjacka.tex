	...
	\chapter{Analiza teoretyczna}
		\section{Haszowanie kukułcze}
			...
			\subsection{Właściwa analiza}
				Przyjrzymy się teraz kanonicznej wersji haszowania kukułczego. Ta wersja różni się nieznacznie od przedstawionej wcześniej w konteście LogStore'a (\ref{logstore}) -- zamiast jednej tablicy mamy dwie ($T_1$ i $T_2$), każda z nich jest przypisana do konkretnej funkcji haszującej (odpowiednio $h_1$ i $h_2$). Obie wersje są sobie równoważne, ale z punktu widzenia analizy złożoności wersja z dwiema tablicami będzie wygodniejsza. Dalsza część analizy bazuje na pracy \cite{CH} i wykładzie \cite{UW}\\
				\indent Niech $n$ oznacza liczbę elementów, które chcemy umieścić w tablicy z haszowaniem. Rozważmy dwudzielny graf $G=(V,E)$, którego wierzchołki odpowiadają komórkom tablic $T_1$ i $T_2$. Krawędzie w grafie odpowiadają kubełkowi oraz alternatywnemu kubełkowi. Innymi słowy dla danego klucza $k$ w grafie $G$ istnieje krawędź $(T_1[h_1(k)], T_2[h_2(k)])$, gdzie $h_1, h_2 : U\to\left\{0,\ldots, t - 1\right\}$ to funkcje haszujące pochodzące z $(1, MaxLoop)$-uniwersalnej rodziny funkcji haszujących\cite{CH}. $MaxLoop$ oznacza ograniczenie na liczbę iteracji.\\
				\indent Niech $b_1,\ldots,b_i$ będzie ciągiem wierzchołków odwiedzanych przez klucz $k$ podczas wstawiania go do tablicy z haszowaniem kukułczym. Zauważmy, że wyznacza on w grafie $G$ pewną ścieżkę $S$, niekoniecznie prostą (dopuszczamy powtarzające się wierzchołki).
				\ble
				...
				\ele
				\bdd
					Jeśli w wyniku wypychania kolejnych kluczy odwiedzamy różne kubełki i na końcu trafiamy na pusty, to udało nam się wstawić. Jeśli wróciliśmy do jakiegoś kubełka, to tym razem klucz $k$ zostanie przerzucony do alternatywnego kubełka, a skoro nie ma więcej cykli, to podążając tamtą ścieżką uda nam się wstawić ten klucz do tablicy.
				\edd
				\ble
					Jeśli $k$ jest kluczem wstawianym do tablicy z haszowaniem kukułczym, to wstawienie nie powiedzie się, gdy występują $2$ lub więcej cykle w $S$.
				\ele
				\bdd
					W procesie wstawiania każda krawędź na ścieżce $S$ odpowiada kluczowi, dla którego należy znaleźć wolny kubełek. Skoro w spójnej składowej o $r$ wierzchołkach, wyznaczonej przez ścieżkę, znajdują się dwa cykle, to musi w niej też być przynajmniej $r + 1$ krawędzi. W związku z tym mamy więcej kluczy do wstawienia (przynajmniej $r + 1$) niż wierzchołków (kubełków, dokładnie $r$).
				\edd
				\ble
					Prawdopodobieństwo wystąpienia co najwyżej jednego cyklu na ścieżce $S$ jest ograniczone przez $2 \cdot \left(\frac{n}{t}\right)^{\frac{r}{3} - 1}$.
				\ele
				\ble
					Prawdopodobieństwo wystąpienia dwóch cykli na ścieżce $S$ jest ograniczone przez $O\left(\frac{1}{n^2}\right)$.
				\ele
				Oba oszacowania pochodzą z \cite{CH}. Przykładowo przyjrzyjmy się dowodowi ostatniego lematu.
				\bdd
					Rozważmy ścieżkę długości $r$. Chcemy oszacować liczbę możliwych wystąpień dwóch cykli.\\
					\indent Niech $b_1$ będzie ustalonym wierzchołkiem startowym. Na $n^{r - 1}$ sposobów wybieramy pozostałe wierzchołki na ścieżce, na $r^3$ sposobów określamy możliwe wybory indeksów dla wierzchołków zamykających cykl oraz na $t^{r - 1}$ sposobów wybieramy wierzchołki grafu, przez które będzie przechodził cykl. Dla jednego z nich nie ma miejsca, więc musimy odjąć jeden od $r$. Otrzymujemy oszacowanie na liczbę cykli równe $n^{r - 1}\cdot r^3\cdot t^{r - 1}$.\\
					\indent Zauważmy jednak, że każda krawędź może znaleźć się na cyklu z prawdopodobieństwem $\left(\frac{1}{t}\right)^2$. Co daje nam oszacowanie $\frac{n^{r - 1}\cdot r^3\cdot t^{r - 1}}{t^{2r}}$. Aby otrzymać ostateczne oszacowanie obliczamy sumę po wszystkich możliwych wartościach $r$
					$$
						\displaystyle\sum_{r\geq 3} \frac{n^{r - 1}\cdot r^3\cdot t^{r - 1}}{t^{2r}} \leq O\left(\frac{1}{n^2}\right)
					$$
				\edd
				\btw
				...
				\etw
				\bdd
					Czas wstawiania możemy obliczyć stosując wzór na prawdopodobieństwo warunkowe. Obliczamy czas potrzebny w sytuacji, w której nie ma potrzeby przehaszowania tablicy. Wynosi on $O(1) \cdot \left(1 - O\left(\frac{1}{n^2}\right)\right)$. Oszacowanie na koszt pojedyńczej operacji wynika z obliczenia sumy $\displaystyle\sum_r 2 \cdot \left(\frac{n}{t}\right)^{\frac{r}{3} - 1}$, którą można oszacować przez $O\left(\displaystyle\sum_r \left(\frac{1}{2}\right)^{\frac{r}{3} - 2}\right) = O(1)$. Czas w przypadku konieczności ponownego haszowania wynosi $O(n\log n)\cdot O\left(\frac{1}{n^2}\right)$. Oszacowanie na koszt pojedyńczej operacji wynika z oszacowania na wartość $MaxLoop$, które jest równe $O(\log n)$.\\
