	...
	\chapter{Analiza teoretyczna}
		\section{Haszowanie kukułcze}
			...
			\subsection{Właściwa analiza}
				...
				\efa
				\bsd
					Jeśli w~wyniku wypchnięcia kolejnych kluczy odwiedzamy różne kubełki i~na końcu trafiamy na pusty, to udało nam się wstawić. Jeśli wróciliśmy do jakiegoś kubełka (mamy cykl), to tym razem kolejne klucze będą wypychane w odwrotnej kolejności (przywracając poprzedni stan), a na końcu klucz $k$ będzie wypchnięty do początkowego kubełka. Wówczas zostanie wstawiony do alternatywnego kubełka, a~skoro nie ma więcej cykli, to kolejne wypchnięcia doprowadzą do pomyślnego wstawienia klucza do słownika.
				\esd
				\bfa
				...
				\bsd
					...
					$$
						\displaystyle\sum_{s\geq 3}\frac{n^{s - 1}\cdot s^3\cdot t^{s - 1}}{t^{2s}}\leq\frac{1}{tn}\displaystyle\sum_{s\geq 3} s^3\cdot\left(\frac{n}{t}\right)^s\leq O\left(\frac{1}{n^2}\right)
					$$
				\esd
				\btw
